...
//   2) 原句必配译句、译句非空；
//   3) 36 章（系辞上 12 / 系辞下 13 / 说卦 11）一章不缺、无重复；
...
for (const raw of readFileSync(SRC, 'utf8').split(/\r?\n/)) {
  ...
  if (!m) { console.error('无法识别的行（' + SRC + '）：' + line); process.exit(1) }
  if (m[1] === '原｜') {
...
const want = {}
for (const b of [...getXici(), { book: '说卦传', chapters: SHUOGUA }]) {
