...
const HEAD = /^(.+?)｜(卦辞|初六|初九|九二|六二|九三|六三|九四|六四|九五|六五|上九|上六)｜(.+)$/
...
for (const [k, e] of map) {
  ...
  if (e.ti !== '卦辞' && !g.yaoci.some(y => y.ti === e.ti)) { console.error(`✗ 无此爻：${e.name}｜${e.ti}`); dropped++; continue }
  out.push({ key: g.key, ti: e.ti, gu: e.gu, lai: e.lai })
}
out.sort((a, b) => GUA_LIST.findIndex(g => g.key === a.key) - GUA_LIST.findIndex(g => g.key === b.key) || (a.ti === '卦辞' ? -1 : b.ti === '卦辞' ? 1 : 0))
