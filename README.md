...
- 当前版本：**v0.4.0**（云端切换轮 · tag `v0.4.0` · 2026-08-27）
...
1. **敏感违规处理（最高优先）**：小程序疑似被平台整包标记，分享被拦——查公众平台违规记录、对症申诉/整改后再提审
2. 卦爻辞白话 56 卦**外审闭环**（内容已入库 64/64，`docs/待填充-白话翻译.txt` 为素材档；外审修正后 `convert-baihua.mjs` 重并，txt 为准）
3. 正式提审材料：隐私声明、类目核对（文化学习类）
4. BGM 音源未配置（`packageBooks/utils/bgm.js` 预留接口，`BGM_SRC` 为空）
5. 真机回归清单逐轮走查（`docs/真机回归清单.md`）
