...
3. **部署**：IDE 中右键 `cloudfunctions/wenyi` →「上传并部署：云端安装依赖」
   ...
   >60s 必超函数超时，**勿设 0**；`WENYI_DAILY_LIMIT` 每用户每日云端
   次数上限，缺省 10、显式 0 关闭——计数存云数据库 `wenyi_quota` 集合，
   首调自动建，超限返回 `LIMIT` 端上降级本地合成）→ 确认超时 60s
   （config.json 已带）；
...
- ~~v0.4.0 后若接流控/计费告警，可在 index.js 加简单限流（openid 每日次数）~~
  **已落地（v0.4.0 后续批）**：`quota.js` openid 每日计数（默认 10，
  `WENYI_DAILY_LIMIT` 可调），超限返回 `LIMIT`、端上明示后降级本地合成；
- ~~免责声明措辞、AIGC 合规标识（平台要求生成内容标识时）在提审前复核~~
  **已落地（v0.4.0 后续批）**：云端解读摘要行标「AI 生成」、页脚
  「问易解读由 AI 生成，仅供传统文化学习参考 · 不构成任何决策依据」；
  模拟态标「本地合成（模拟）」——两种来源均显式标识，提审材料对应项。
