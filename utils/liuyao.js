...
// 八卦顺序（用于卦名矩阵行列索引）
export const BAGUA_ORDER = ['乾', '兑', '离', '震', '巽', '坎', '艮', '坤']
...
export function kongWangByGanZhiIdx(idx) {
...
export function ganZhiToIdx(gan, zhi) {
...
export function dateToGanZhi(date) {
...
// 返回：完整排盘对象
export function paipan({ yao, dayGan, dayZhi }) {
...
export default { paipan, dateToGanZhi, wuxingRelation, kongWangByGanZhiIdx }

// ============ 64 卦查询/列举（供知识库生成与查表）============
// 由 6 位 key(初→上，'1'=阳'0'=阴) 得到卦信息
export function hexagramFromKey(key) {
...
export function listAllHexagrams() {
